...
**Important**: All scripts are located in the `scripts/x/` subdirectory of this skill.

**Agent Execution Instructions**:
1. Determine this SKILL.md file's directory path as `{baseDir}`
2. Script path = `{baseDir}/scripts/x/<script-name>.ts`
...
| Script | Purpose |
|--------|---------|
| `scripts/x/x-browser.ts` | Regular posts (text + images) |
| `scripts/x/x-video.ts` | Video posts (text + video) |
| `scripts/x/x-quote.ts` | Quote tweet with comment |
| `scripts/x/x-article.ts` | Long-form article publishing (Markdown) |
| `scripts/x/md-to-html.ts` | Markdown → HTML conversion |
| `scripts/x/copy-to-clipboard.ts` | Copy content to clipboard |
| `scripts/x/paste-from-clipboard.ts` | Send real paste keystroke |
| `scripts/x/check-paste-permissions.ts` | Verify environment & permissions |
...
```bash
${BUN_X} {baseDir}/scripts/x/check-paste-permissions.ts
```
...
```bash
${BUN_X} {baseDir}/scripts/x/x-browser.ts "Hello!" --image ./photo.png
```
...
```bash
${BUN_X} {baseDir}/scripts/x/x-video.ts "Check this out!" --video ./clip.mp4
```
...
```bash
${BUN_X} {baseDir}/scripts/x/x-quote.ts https://x.com/user/status/123 "Great insight!"
```
...
```bash
${BUN_X} {baseDir}/scripts/x/x-article.ts article.md
${BUN_X} {baseDir}/scripts/x/x-article.ts article.md --cover ./cover.jpg
```
