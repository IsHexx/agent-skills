...
**Agent Execution**: Determine this SKILL.md directory as `{baseDir}`, then use `{baseDir}/scripts/wechat/<name>.ts`. Resolve `${BUN_X}` runtime: if `bun` installed → `bun`; if `npx` available → `npx -y bun`; else suggest installing bun.

| Script | Purpose |
|--------|---------|
| `scripts/wechat/wechat-browser.ts` | Image-text posts (图文) |
| `scripts/wechat/wechat-article.ts` | Article posting via browser (文章) |
| `scripts/wechat/wechat-api.ts` | Article posting via API (文章) |
| `scripts/wechat/md-to-wechat.ts` | Markdown → WeChat-ready HTML with image placeholders |
| `scripts/wechat/check-permissions.ts` | Verify environment & permissions |
...
```bash
${BUN_X} {baseDir}/scripts/wechat/wechat-api.ts <file> --theme default --account ai-tools
${BUN_X} {baseDir}/scripts/wechat/wechat-article.ts --markdown <file> --theme default --account baoyu
${BUN_X} {baseDir}/scripts/wechat/wechat-browser.ts --markdown <file> --images ./photos/ --account baoyu
```
...
```bash
${BUN_X} {baseDir}/scripts/wechat/check-permissions.ts
```
...
```bash
${BUN_X} {baseDir}/scripts/wechat/wechat-browser.ts --markdown article.md --images ./images/
${BUN_X} {baseDir}/scripts/wechat/wechat-browser.ts --title "标题" --content "内容" --image img.png --submit
```
...
```bash
${BUN_X} {baseDir}/scripts/wechat/wechat-api.ts <file> --theme <theme> [--color <color>] [--title <title>] [--summary <summary>] [--author <author>] [--cover <cover_path>] [--no-cite]
```
...
```bash
${BUN_X} {baseDir}/scripts/wechat/wechat-article.ts --markdown <markdown_file> --theme <theme> [--color <color>] [--no-cite]
${BUN_X} {baseDir}/scripts/wechat/wechat-article.ts --html <html_file>
```
