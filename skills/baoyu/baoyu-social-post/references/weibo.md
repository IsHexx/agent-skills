...
**Important**: All scripts are located in the `scripts/weibo/` subdirectory of this skill.

**Agent Execution Instructions**:
1. Determine this SKILL.md file's directory path as `{baseDir}`
2. Script path = `{baseDir}/scripts/weibo/<script-name>.ts`
...
| Script | Purpose |
|--------|---------|
| `scripts/weibo/weibo-post.ts` | Regular posts (text + images) |
| `scripts/weibo/weibo-article.ts` | Headline article publishing (Markdown) |
| `scripts/weibo/copy-to-clipboard.ts` | Copy content to clipboard |
| `scripts/weibo/paste-from-clipboard.ts` | Send real paste keystroke |
...
```bash
${BUN_X} {baseDir}/scripts/weibo/weibo-post.ts "Hello Weibo!" --image ./photo.png
${BUN_X} {baseDir}/scripts/weibo/weibo-post.ts "Watch this" --video ./clip.mp4
```
...
```bash
${BUN_X} {baseDir}/scripts/weibo/weibo-article.ts article.md
${BUN_X} {baseDir}/scripts/weibo/weibo-article.ts article.md --cover ./cover.jpg
```
